...
const useAsteroidStore = create((set, get) => ({
  ...
  runSimulation: () => {
    ...
    const computedDistance =
      Number(s.distance) ||
      Math.sqrt((Number(s.xdistance) || 0) ** 2 + (Number(s.ydistance) || 0) ** 2 + (Number(s.zdistance) || 0) ** 2) ||
      1;

    const typeMultiplier = s.type === 'A' ? 1.2 : s.type === 'B' ? 1.5 : 1;
    const damage = ((speed * size) / Math.max(computedDistance, 1)) * typeMultiplier;
    ...
    console.log('runSimulation →', { speed, size, computedDistance, damage });
    return damage;
  },
  ...
  fetchAsteroids: async () => {
    ...
    try {
      ...
      console.log(`fetchAsteroids: loaded ${res.data?.length ?? 0}`);
    } catch (err) {
  ...
  fetchAsteroidDetails: async (name) => {
    if (!name) return;
    try {
      const res = await axios.get(`${BACKEND_BASE}/api/asteroids/${encodeURIComponent(name)}`);
      const data = res.data;

      // Use size directly from API (diameter/2)
      ...
      set({
        ...
        size,
        xdistance: 100, // constant
        ydistance: 50,  // constant
        zdistance: 0,   // constant
        distance: Number(data.missDistance) || 0,
      });

      console.log('fetchAsteroidDetails -> set state for', name, { speed: data.speed, size, position: { x:100, y:50, z:0 } });
    } catch (err) {
